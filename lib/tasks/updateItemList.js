...
module.exports = async (transaction) => {
  ...
  while(!finished) {
    ...
    for (let smItem of request.results) {
      ...
      console.log(`updateItemList... item: ${smItem.name}; count: ${count}/${request.total}; req: ${page}/${Math.ceil(request.total / 100)}`.yellow);

      let update = false

      for (attribute of item._options.attributes) {
        if (item[attribute] == null) {
          if (!update) update = {};

          update[attribute] = smItem[attribute];
        }
      }
