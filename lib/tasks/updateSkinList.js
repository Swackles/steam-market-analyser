  ...
  while(!finished) {
    ...
    for (let smSkin of request.results) {
      ...
      if (skin) {
        ...
        const properties = await smSkin.getAllProperties()

        smSkin.nameId = properties.nameId;
        smSkin.description = properties.description

        let item = await Item.findOrCreate({name: properties.parentItem}, {transaction: transaction});

        smSkin.itemId = item.id
