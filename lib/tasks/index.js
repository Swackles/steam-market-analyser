...
module.exports = async (func) => {
  ...
  try {
    ...
    await TaskLog.upsert({status: 'COMPLETED', description: `Completed in ${time}`}, {where: {id: log.dataValues.id}});
    ...
    if (err instanceof Error && err.code === "MODULE_NOT_FOUND") {
      ...
      await TaskLog.upsert({status: 'FAILED', description: `Module not found`}, {where: {id: log.dataValues.id}});
    } else {

      console.log(err);
      await TaskLog.upsert({status: 'FAILED', description: `${err}`}, {where: {id: log.dataValues.id}});
